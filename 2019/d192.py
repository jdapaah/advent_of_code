def main():
	...
	while r:
		print(r, end=', ')
		for x, y in [(i, r) for i in range(r + 1)] + [(r, j) for j in range(r)]:
			done = True
			if (x, y) not in grid:
				grid[x, y] = p1.out(program[:], (x, y))
				p1.reset()
			for i in range(100):
				if (x + i, y) not in grid:
					grid[x + i, y] = p1.out(program[:], (x + i, y))
					p1.reset()
				if (x, y + i) not in grid:
					grid[x, y + i] = p1.out(program[:], (x, y + i))
					p1.reset()

				if grid[x + i, y] == 0 or grid[x, y + i] == 0:
					done = False
					break
			if done:
				print('\n', x, y)
